...
format_percented_string(const char *str, problem_data_t *pd, FILE *result)
{
    ...
    while (*str) {
        ...
        case '%': ;
            ...
            *nextpercent = '\0';
            const problem_item *item = get_problem_data_item_or_NULL(pd, str);
            *nextpercent = '%';
...
append_short_backtrace(struct strbuf *result, problem_data_t *problem_data, size_t max_text_size, bool print_item_name)
{
    ...
    {
        VERB3 log("'backtrace' exceeds the text file size, going to append its short version");

        char *error_msg = NULL;
        const char *analyzer = get_problem_item_content_or_NULL(problem_data, FILENAME_ANALYZER);
        if (!analyzer)
        {
            VERB3 log("Problem data does not contain '"FILENAME_ANALYZER"' file");
            return 0;
        }
        ...
        {
            VERB3 log("Successfully identified 'CCpp' abrt type");
    ...
    {
        VERB3 log("'backtrace' is small enough to be included as is");
    }
...
append_item(struct strbuf *result, const char *item_name, problem_data_t *pd, GList *comment_fmt_spec)
{
    ...
    if (item_name[0] != '%')
    {
        struct problem_item *item = get_problem_data_item_or_NULL(pd, item_name);
        if (!item)
            return 0; /* "I did not print anything" */
        if (!(item->flags & CD_FLAG_TXT))
            return 0; /* "I did not print anything" */
...
get_attached_files(problem_data_t *pd, GList *items, GList *comment_fmt_spec)
{
    ...
    while (item != NULL)
    {
        ...
        if (special == NULL)
        {
            VERB1 log("No attachment found for '%s'", item_name);
            continue;
        }
...
problem_report_add_custom_section(problem_report_t *self, const char *name)
{
    ...
    {
        log("Custom section already exists : '%s'", name);
        return -EEXIST;
    }

    VERB3 log("Problem report enriched with section : '%s'", name);
...
problem_report_get_section_buffer(const problem_report_t *self, const char *section_name)
{
    if (self->pr_sec_custom == NULL)
    {
        VERB3 log("Couldn't find section '%s': no custom section added", section_name);
        return NULL;
    }
...
problem_formatter_add_section(problem_formatter_t *self, const char *name, int flags)
{
    ...
    {
        VERB3 log("Extra section already exists : '%s' ", name);
        return -EEXIST;
    }
...
problem_formatter_validate(problem_formatter_t *self)
{
    ...
    for (GList *iter = self->pf_extra_sections; iter; iter = g_list_next(iter))
    {
        ...
        VERB3 log("Validating extra section : '%s'", section->pfes_name);
        ...
        {
            log("Problem format misses required section : '%s'", section->pfes_name);
            ++retval;
        }
    }
    ...
    for (GList *iter = self->pf_sections; iter; iter = g_list_next(iter))
    {
        ...
        {
            log("Problem format contains unrecognized section : '%s'", section->name);
            ++retval;
        }
    }
...
problem_formatter_generate_report(const problem_formatter_t *self, problem_data_t *data, problem_report_t **report)
{
    ...
    for (GList *iter = self->pf_sections; iter; iter = g_list_next(iter))
    {
        ...
        else /* %description or a custom section (e.g. %additional_info) */
        {
            ...
            if (buffer != NULL)
            {
                VERB3 log("Formatting section : '%s'", section->name);
                format_section(section, data, self->pf_sections, buffer);
            }
            else
                log("Unsupported section '%s'", section->name);
        }
    }

    if (!has_summary) {
        VERB3 log("Problem format misses section '%%summary'. Using the default one : '%s'.",
